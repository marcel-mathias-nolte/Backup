...
namespace Nolte.Security
{
    public class UAC
    {
                ...
                try
                {
                    ...
                    switch (IL)
                    {
                        case NativeMethods.SECURITY_MANDATORY_UNTRUSTED_RID:
                            IntegrityLevel = "Untrusted"; break;
                        case NativeMethods.SECURITY_MANDATORY_LOW_RID:
                            IntegrityLevel = "Low"; break;
                        case NativeMethods.SECURITY_MANDATORY_MEDIUM_RID:
                            IntegrityLevel = "Medium"; break;
                        case NativeMethods.SECURITY_MANDATORY_HIGH_RID:
                            IntegrityLevel = "High"; break;
                        case NativeMethods.SECURITY_MANDATORY_SYSTEM_RID:
                            IntegrityLevel = "System"; break;
                        default:
                            IntegrityLevel = "Unknown"; break;
                    }
                }
                catch (Exception ex)
                {
                    IntegrityLevel = "N/A";
                    Debug.WriteLine("An error occurred in GetProcessIntegrityLevel:\n" + ex.Message);
                }
            }
            else
            {
        ...
        public static string IntegrityLevel { get; private set; }

        public static void CreateElevateButton(Button TargetButton)
        {
            if (Environment.OSVersion.Version.Major >= 6)
            {
                // Running Windows Vista or later (major version >= 6). 
                ...
                try
                {
                    ...
                    NativeMethods.SendMessage(TargetButton.Handle,
                        NativeMethods.BCM_SETSHIELD, 0,
                        fIsElevated ? IntPtr.Zero : (IntPtr)1);
                    TargetButton.Click += DoElevate;
                }
                catch (Exception ex)
                {
        ...
        public static void DoElevate(object sender = null, EventArgs e = null)
        {
