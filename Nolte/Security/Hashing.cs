...
namespace Nolte.Security
{
    class Hashing
    {
        ...
        {
            byte[] encodedDataAsBytes = System.Convert.FromBase64String(encodedData.Trim(new char[] { '\0', ' ' }));
            return System.Text.Encoding.Default.GetString(encodedDataAsBytes);
        }

        public static string ImageToString(System.Drawing.Image im)
        {
            if (im == null)
            {
                im = new System.Drawing.Bitmap(1, 1);
            }
            MemoryStream ms = new MemoryStream();
            im.Save(ms, new System.Drawing.Imaging.ImageFormat(Settings.Default.ImageDefaultFormat));
        ...
        {
            byte[] array = Convert.FromBase64String(imageString.Trim(new char[] { '\0', ' ' }));
            return System.Drawing.Image.FromStream(new MemoryStream(array));
        }
    }
}
