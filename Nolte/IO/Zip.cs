...
namespace System.IO
{
    ...
    public class Zip : IDisposable
    {
        ...
        private void AddRecursive(Compression _method, string _pathname, string _filenameInZip, string _comment)
        {
            string rel;
            foreach (string f in System.IO.Directory.GetFiles(_pathname))
            {
                try
                {
                    rel = _filenameInZip + "\\" + f.Substring(f.LastIndexOf('\\') + 1).TrimStart('\\');
                    using (FileStream stream = new FileStream(f, FileMode.Open, FileAccess.Read))
                    {
                        AddStream(_method, rel, stream, File.GetLastWriteTime(f), _comment);
                        stream.Close();
                    }
                }
                catch
            ...
            foreach (string d in System.IO.Directory.GetDirectories(_pathname))
            {
                rel = _filenameInZip + "\\" + d.Substring(d.LastIndexOf('\\') + 1).TrimStart('\\');
                AddRecursive(_method, d, rel, _comment);
            }
        }
        public void AddFolder(Compression _method, string _pathname, string _comment)
        {
            if (Access == FileAccess.Read)
                throw new InvalidOperationException("Writing is not alowed");
            AddRecursive(_method, _pathname, "", _comment);
        }
        /// <summary>
